...
class ConvGnT(object):
    ...
    def __init__(self, net, hidden_activation, opt, num_last_filter_outputs=4,
                decay_rate=0.99, replacement_rate=1e-4, init='kaiming',
                util_type='contribution', maturity_threshold=100):
        ...
        for i in range(self.num_hidden_layers):
            if isinstance(self.net[i * 2], Conv2d):
                self.util.append(zeros(self.net[i * 2].out_channels, device=self.dev))
                self.bias_corrected_util.append(zeros(self.net[i * 2].out_channels, device=self.dev))
                self.ages.append(zeros(self.net[i * 2].out_channels, device=self.dev))
                self.mean_feature_act.append(zeros(self.net[i * 2].out_channels, device=self.dev))
                self.mean_abs_feature_act.append(zeros(self.net[i * 2].out_channels, device=self.dev))
            elif isinstance(self.net[i * 2], Linear):
                self.util.append(zeros(self.net[i * 2].out_features, device=self.dev))
                self.bias_corrected_util.append(zeros(self.net[i * 2].out_features, device=self.dev))
                self.ages.append(zeros(self.net[i * 2].out_features, device=self.dev))
                self.mean_feature_act.append(zeros(self.net[i * 2].out_features, device=self.dev))
                self.mean_abs_feature_act.append(zeros(self.net[i * 2].out_features, device=self.dev))
